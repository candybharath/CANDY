...
from info import LANGUAGES, ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, LOG_CHANNEL, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, IMDB_DLT_TIME, FILE_FORWARD, FILE_CHANNEL, MAIN_CHANNEL, SP, LOGIN_CHANNEL, CUSTOM_QUERY_CAPTION, VERIFY, QUERY_TEMPLATE
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, InputMediaPhoto
from pyrogram import Client, filters, enums
from pyrogram.errors import FloodWait, UserIsBlocked, MessageNotModified, PeerIdInvalid
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, mute_login, send_all, check_verification, get_token
...
#    if int(query.from_user.id) not in [query.message.reply_to_message.from_user.id, 0]:
    ...
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="𝐁𝐚𝐜𝐤 𝐓𝐨 𝐅𝐢𝐥𝐞𝐬", callback_data=f"next_{req}_{key}_{offset}")])
...
#        await query.answer(QUERY_TEMPLATE, show_alert=True)
        ...
        await query.answer(script.QUERY_TEMPLATE.format(query.from_user.first_name, user_name= '' if title is None else title),  show_alert=True)
        ...
        buttons = [[
            InlineKeyboardButton('🍷 ᴄʟᴏɴᴇ 🍷', callback_data='delf')
        ], [
        ...
        if settings['auto_delete']:
            btn.insert(0, 
                [
                    ...
                    InlineKeyboardButton(f'{search}', 'imdb')
                ]
            )
        else:
            btn.insert(0, 
